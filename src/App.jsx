  ...
  return (
    <div className="bg-[#050505] text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 bg-black/80 backdrop-blur-lg border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              onClick={goLogin} 
              className="px-6 py-2.5 hover:text-[#67e8f9] transition"
            >
              Log in
            </button>
            <button 
              onClick={goSignup} 
              className="px-8 py-2.5 bg-white text-black rounded-full font-semibold hover:bg-gradient-to-r hover:from-[#67e8f9] hover:to-[#ec4899] hover:text-white transition-all active:scale-95"
            >
              Start Building
            </button>
          </div>
      ...
      <section 
        ...
        style={{ background: "radial-gradient(circle at 50% 30%, rgba(103,232,249,0.22) 0%, transparent 70%)" }}
      >
        <div 
          className="absolute inset-0 pointer-events-none"
          style={{ background: `radial-gradient(circle at 70% ${70 + scrollPosition * 0.05}%, rgba(236,72,153,0.18), transparent)` }} 
        />
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {[
              ...
              <button
                key={item.title}
                onClick={goSignup}
                className="text-left p-10 rounded-3xl border border-white/10 hover:border-[#67e8f9] transition-all group bg-zinc-950 hover:-translate-y-3"
              >
